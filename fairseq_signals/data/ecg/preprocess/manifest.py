...
def main(args):
    ...
    pretrain_subset = get_folder(root_path,  args.pretrain_subset)
    finetune_subset = get_folder(root_path,  args.finetune_subset)
...
def get_folder(root_folder, folders_str):
    final_list = []
    if ':' in folders_str:
        ...
        final_list = [x.strip() for x in folders_str.split(",")]
    
    return [x for x in final_list if os.path.exists(os.path.join(root_folder, x))]
