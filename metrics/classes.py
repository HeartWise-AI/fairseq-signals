...
class Labels77Task(AbstractTask):
    def __init__(self, ds:AbstractDataset, model: AbstractModel, percentage=100):
        ...
        result_key = f'last-ft-labels-77-bce' if percentage == 100 else f'last-ft-labels-77-{percentage}-bce'
...
class Lvef50Task(AbstractTask):
    def __init__(self, ds:AbstractDataset, model: AbstractModel):
        super().__init__('lvef_50', ds, model, 'last-ft-fevg-v2-50-bce')
...
def getSingletonTask(task_key:str, model_key:str, ds_key: str, percentage=100) -> AbstractTask:
...
def get_formated_scores(model_key: str, ds_key: str, task_key: str):
    task = getSingletonTask(task_key, model_key, ds_key)
    return format_scores(task.get_scores())